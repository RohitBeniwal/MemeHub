      ...
      {meme === null ? (
        <>
          ...
          <div className="pagination flex items-center justify-center mb-100 gap-2">
            <button
              className="bg-gray-800 text-white p-1 border border-gray-300 rounded-2 cursor-pointer"
              onClick={prevPage}
            >
              Previous
            </button>
            {Array.from(
              ...
              (_, i) => (
                ...
                className={`bg-white p-1 border border-gray-300 rounded-2 cursor-pointer ${
                  i + 1 === currentPage ? 'text-white bg-gray-800' : '' // Add red color if it's the current page
                }`}
                key={i}
